...
const MobileMainView: React.FC<IProps> = ({ size }) => {
    ...
    const firstStage = (
        <div className="FirstStage">
            <img draggable={false} alt={'main-logo'} src={'ico/main-image-color.png'} />
            <div className="TriangleHorizontal Bottom">
                <div className="TriangleHorizontalContent" />
            </div>
        </div>
    ...
    const secondStage = <div className="SecondStage">{getEditorFeatureTiles(EditorFeatureData.slice(0, 3))}</div>;
    ...
    const fourthStage = (
        <div className="FourthStage">
            <div className="Message">
                Due to the small size of the screen we do not support our editor on mobile devices. Check what you
                missed and visit us from a desktop.
            </div>
            <div className="SocialMediaWrapper">{getSocialMediaButtons({ width: 40, height: 40 })}</div>
        </div>
    );
