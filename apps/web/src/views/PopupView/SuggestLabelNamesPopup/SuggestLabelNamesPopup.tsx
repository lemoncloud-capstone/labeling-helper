...
const SuggestLabelNamesPopup: React.FC<IProps> = ({
    ...
    const onAccept = () => {
        updateLabelNames(
            extractSelectedNames().reduce((acc: LabelName[], entry: string, index: number) => {
                acc.push({
                    name: entry,
                    id: uuidv4(),
    ...
    const selectAll = () => {
        ...
        setLabelNames(
            labelNames.map((entry: SelectableName) => {
                return {
                    ...entry,
                    flag: true,
                };
    ...
    const getOptions = () => {
        return labelNames.map((entry: SelectableName, index: number) => {
            return (
                <div className="OptionsItem" onClick={() => toggleSelectableNameByIndex(index)} key={index}>
                    {entry.flag ? (
        ...
        return (
            <div className="SuggestLabelNamesPopupContent">
                ...
                <div className="AllToggle">
                    <div className="OptionsItem" onClick={() => (selectAllFlag ? deselectAll() : selectAll())}>
                        {selectAllFlag ? (
                            <img draggable={false} src={'ico/checkbox-checked.png'} alt={'checked'} />
                        ) : (
                            <img draggable={false} src={'ico/checkbox-unchecked.png'} alt={'unchecked'} />
                ...
                <div className="LabelNamesContainer">
                    <Scrollbars autoHeight={true}>
                        <div className="LabelNamesContent">{getOptions()}</div>
                    </Scrollbars>
                </div>
            </div>
        );
