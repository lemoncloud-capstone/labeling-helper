...
const ImportLabelPopup: React.FC<IProps> = ({
    ...
    const getOptions = (exportFormatData: ILabelFormatData[]) => {
        return exportFormatData.map((entry: ILabelFormatData) => {
            return (
                <div className="OptionsItem" onClick={() => onAnnotationFormatChange(entry.type)} key={entry.type}>
    ...
    const renderInternalContent = (type: LabelType) => {
        if (!formatType && ImportFormatData[type].length !== 0) {
            return (
                <>
                    <div className="Message">Select file format you would like to use to import labels.</div>,
                    <div className="Options">{getOptions(ImportFormatData[type])}</div>
                </>
            );
        }
    ...
    return (
        <GenericLabelTypePopup
            ...
            disableAcceptButton={
                loadedImageData.length === 0 || loadedLabelNames.length === 0 || !!annotationsLoadedError
            }
            rejectLabel={'Cancel'}
            onReject={onReject}
