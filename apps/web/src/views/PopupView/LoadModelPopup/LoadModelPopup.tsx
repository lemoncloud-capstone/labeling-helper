...
const LoadModelPopup: React.FC<IProps> = ({ updateActivePopupType }) => {
    ...
    const getOptions = () => {
        return selectedModelToLoad.map((entry: SelectableModel) => {
            return (
                <div className="OptionsItem" onClick={() => onSelect(entry.model)} key={entry.model}>
                    {entry.flag ? (
        ...
        return (
            <div className="LoadModelPopupContent">
                <div className="Message">
                    ...
                    your privacy, we decided not to send your images to the server, but instead bring AI to you. Make
                    sure that you have a fast and stable connection - it may take a while to load the model.
                </div>
