...
const GenericLabelTypePopup: React.FC<IProps> = ({
    ...
    const getSidebarButtons = () => {
        return LabelToolkitData.filter((label: ILabelToolkit) => label.projectType === projectType).map(
            (label: ILabelToolkit) => {
                return (
                    <ImageButton
                        key={label.labelType}
    ...
    const renderContent = () => {
        return (
            <div className="GenericLabelTypePopupContent">
                <div className="LeftContainer">{getSidebarButtons()}</div>
                <div className="RightContainer">{renderInternalContent(labelType)}</div>
            </div>
        );
    };
