...
const LoadYOLOv5ModelPopup: React.FC<IProps> = ({ updateActivePopupTypeAction, submitNewNotificationAction }) => {
    ...
    const onDrop = (accepted: File[]) => {
        ...
        if (txtFiles.length === 0) {
            submitNewNotificationAction(
                NotificationUtil.createErrorNotification(NotificationsDataMap[Notification.LABELS_FILE_UPLOAD_ERROR])
            );
        }
    ...
    const onAccept = () => {
        ...
        const onFailure = () => {
            setIsLoading(false);
            const notification =
                modelSource === ModelSource.UPLOAD ? Notification.MODEL_LOAD_ERROR : Notification.MODEL_DOWNLOAD_ERROR;
        ...
        if (modelSource === ModelSource.DOWNLOAD) {
            YOLOV5ObjectDetector.loadModel(
                PretrainedModelDataMap[selectedPretrainedModel].config,
                onSuccess,
                onFailure
            );
        } else {
    ...
    const renderOptions = () => {
        return <div className="options">{getOptionsContent()}</div>;
    };

    const renderMessage = () => {
        const uploadMessage: string =
            'Drag and drop your own YOLOv5 model converted to tensorflow.js format and ' +
        ...
        return <div className="message">{modelSource === ModelSource.DOWNLOAD ? downloadMessage : uploadMessage}</div>;
    };

    const renderLoader = () => {
        return (
            <div className="loader">
                <ClipLoader size={40} color={CSSHelper.getLeadingColor()} loading={true} />
            </div>
        );
    };
    ...
    const renderDropZone = () => {
        return <div {...getRootProps({ className: 'drop-zone' })}>{getDropZoneContent()}</div>;
    };
    ...
    const renderContent = () => {
        ...
        return (
            <div className="load-yolo-v5-model-popup">
                {renderMenu()}
    ...
    const disableAcceptButton =
        modelSource === ModelSource.UPLOAD && (modelFiles.length === 0 || classNames.length === 0);
...
export default connect(mapStateToProps, mapDispatchToProps)(LoadYOLOv5ModelPopup);
