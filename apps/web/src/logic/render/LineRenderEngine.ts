...
export class LineRenderEngine extends BaseRenderEngine {
    ...
    private drawLine(labelId: string, line: ILine, isActive: boolean) {
        ...
        if (isActive) {
            LineUtil.getPoints(line).forEach((point: IPoint) =>
        ...
        imageData.labelLines = imageData.labelLines.map((lineLabel: LabelLine) => {
            if (lineLabel.id !== activeLabel.id) {
                ...
                return {
                    ...
                    line: {
                        start:
                            this.lineUpdateAnchorType === LineAnchorType.START
                                ? mousePositionOnImage
                                : lineLabel.line.start,
                        end:
                            this.lineUpdateAnchorType === LineAnchorType.END
                                ? mousePositionOnImage
                                : lineLabel.line.end,
                    },
                };
    ...
    private getLineUnderMouse(data: EditorData): LabelLine | null {
        ...
        const labelLines: LabelLine[] = LabelsSelector.getActiveImageData().labelLines.filter(
            (labelLine: LabelLine) => labelLine.isVisible
        );
