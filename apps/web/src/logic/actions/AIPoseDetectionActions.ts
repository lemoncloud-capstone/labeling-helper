...
export class AIPoseDetectionActions {
    ...
    public static detectPoses(imageId: string, image: HTMLImageElement): void {
        if (
            LabelsSelector.getImageDataById(imageId).isVisitedByPoseDetector ||
        ...
        PoseDetector.predict(image, (poses: Pose[]) => {
            const suggestedLabelNames = AIPoseDetectionActions.extractNewSuggestedLabelNames(
    ...
    private static mapPredictionsToPointLabels(predictions: Pose[]): LabelPoint[] {
        return predictions
            .map((prediction: Pose) => {
                return prediction.keypoints.map((keypoint: Keypoint) => {
                    return {
                        id: uuidv4(),
                        labelIndex: null,
                        labelId: null,
    ...
    public static rejectAllSuggestedPointLabels(imageData: ImageData) {
        const newImageData: ImageData = {
            ...
            labelPoints: imageData.labelPoints.filter(
                (labelPoint: LabelPoint) => labelPoint.status === LabelStatus.ACCEPTED
            ),
        };
        store.dispatch(updateImageDataById(newImageData.id, newImageData));
    }
