...
export class EditorActions {
    ...
    public static updateMousePositionIndicator(event: React.MouseEvent<HTMLCanvasElement, MouseEvent> | MouseEvent) {
        ...
        if (isMouseOverViewPort && !GeneralSelector.getPreventCustomCursorStatus()) {
            ...
            if (
                isMouseOverImage &&
                ![CustomCursorStyle.GRAB, CustomCursorStyle.GRABBING].includes(GeneralSelector.getCustomCursorStyle())
            ) {
                ...
                const text: string =
                    'x: ' + Math.round(mousePositionOverImage.x) + ', y: ' + Math.round(mousePositionOverImage.y);
