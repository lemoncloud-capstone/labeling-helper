...
export class AIRoboflowAPIObjectDetectionActions {
    public static detectRects(imageData: ImageData): void {
        if (imageData.isVisitedByRoboflowAPI || !AISelector.isRoboflowAPIModelLoaded()) return;
        ...
        RoboflowAPIObjectDetector.predict(imageData, (predictions: DetectedObject[]) => {
            const suggestedLabelNames = AIRoboflowAPIObjectDetectionActions.extractNewSuggestedLabelNames(
    ...
    public static saveRectPredictions(imageData: ImageData, predictions: DetectedObject[]) {
        const predictedLabels: LabelRect[] =
            AIRoboflowAPIObjectDetectionActions.mapPredictionsToRectLabels(predictions);
