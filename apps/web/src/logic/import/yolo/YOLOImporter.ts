...
export class YOLOImporter extends AnnotationImporter {
    ...
    public import(
        ...
        try {
            const sourceImagesData = LabelsSelector.getImagesData().map((i: ImageData) =>
                ImageDataUtil.cleanAnnotations(i)
            );
            const { labelNameFile, annotationFiles } = YOLOImporter.filterFilesData(filesData, sourceImagesData);
            const [relevantImageData, relevantAnnotations] = YOLOImporter.matchImagesWithAnnotations(
                sourceImagesData,
                annotationFiles
            );
            const labelNamesPromise: Promise<LabelName[]> = FileUtil.readFile(labelNameFile).then(
                (fileContent: string) => YOLOUtils.parseLabelsNamesFromString(fileContent)
            );
            ...
            Promise.all([labelNamesPromise, missingImagesPromise, annotationFilesPromise])
                .then((values: [LabelName[], void, string[]]) => {
                    const [labelNames, , annotationsRaw] = values;
                    const resultImageData = zip<ImageData, string>(relevantImageData, annotationsRaw).map(
                        (pair: [ImageData, string]) => YOLOImporter.applyAnnotations(pair[0], pair[1], labelNames)
                    );
                    onSuccess(
    ...
    public static filterFilesData(filesData: File[], imagesData: ImageData[]): YOLOFilesSpec {
        ...
        const matchingPartitionResult = ArrayUtil.partition(filesData, (i: File) =>
    ...
    public static matchImagesWithAnnotations(images: ImageData[], annotations: File[]): [ImageData[], File[]] {
        ...
        return ArrayUtil.unzip(ArrayUtil.match<ImageData, File>(images, annotations, predicate));
    }
