...
describe('ArrayUtil partition method', () => {
    ...
    it('should pass items with key name from list', () => {
        ...
        const items = [{ name: 'aaa' }, { name: 'aab' }, { name: 'abb' }, { name: 'bbb' }];
...
describe('ArrayUtil getByInfiniteIndex method', () => {
    it('should throw EmptyArrayError', () => {
        // then
        expect(() => {
            ArrayUtil.getByInfiniteIndex([], 0);
        }).toThrowError(new EmptyArrayError());
    });

    it('should throw NegativeIndexError', () => {
        // then
        expect(() => {
            ArrayUtil.getByInfiniteIndex([1], -1);
        }).toThrowError(new NegativeIndexError());
    });
